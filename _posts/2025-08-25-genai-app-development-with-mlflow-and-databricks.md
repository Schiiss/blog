...
Building GenAI apps can introduce some interesting challenges into your software development. Unlike your software throwing an exception (ie: a failure), things like *hallucinations*, where the app returns wrong information—could give the end user the impression of the app 'working' when in reality that is not the case. The inputs to the GenAI apps are generally endless and the potential input can be vast.
